...
def test_loss(ap_distance, triplet_file):
  tripletImages=[]
  triplet = [np.load(triplet_file[0]) , np.load(triplet_file[1]),np.load(triplet_file[2])]
  tripletImages.append(triplet)
  tripletImages = np.array(tripletImages)
  an_distance = np.linalg.norm(tripletImages[0,0]- tripletImages[0,2])
  loss = ap_distance - an_distance
  loss = max(loss + 15, 0.0)
  if loss > 0:
    #print("Hereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
    #print("A_P_distance : " + str(ap_distance) + " || A_N_distance : " + str(an_distance) + " || Loss : " + str(loss))
    return loss
  else:
    loss = 0
  return loss
      ...
      for idxB in range(idxA+1,len(images)):
        ...
        if (label == label_):
            ...
            for s in negIdx:
              ...
              loss = test_loss(ap_distance, tripletImage)
              if loss != 0:
                #print(negImage)
                ...
                cnt +=1
            aux_cnt += 1
            print("Counter: " + str(aux_cnt))
  return (np.array(tripletImages))


triplet_train  = make_triplets(x_train, labels_train,0)
triplet_val = make_triplets(x_val, labels_val,1)

print(len(triplet_train))
print(len(triplet_val))

pickle_out = open("/home/fproenca/Tese/Data/CUHK01/triplet_train.pickle","wb")
pickle.dump(triplet_train, pickle_out)
pickle_out.close()

pickle_out = open("/home/fproenca/Tese/Data/CUHK01/triplet_val.pickle","wb")
